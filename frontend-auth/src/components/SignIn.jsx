...
const SignIn = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (signin.fulfilled.match(result)) {
      ...
      // 1200ms (1.2s) delay to ensure KMS-encrypted cookies are fully processed before navigation
  ...
  const handleGoogleSignIn = () => {
     ...
     * 
     * Initiates Google OAuth flow by redirecting to our API Gateway endpoint.
     * The flow follows this secure pattern:
     * 
     * 1. User clicks "Sign in with Google" → Browser redirects to /auth/google
     * 2. API Gateway Lambda redirects to Cognito hosted UI for Google OAuth
     * 3. User authenticates with Google → Google redirects to Cognito
     * 4. Cognito processes OAuth tokens → Redirects to /auth/google/callback
     * 5. Callback Lambda sets httpOnly cookies → Redirects to dashboard
     * 
     * SECURITY BENEFITS:
     * - No client-side token handling (httpOnly cookies only)
     * - Server-side OAuth token exchange (secure)
